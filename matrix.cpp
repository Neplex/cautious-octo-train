...
Matrix::Matrix(unsigned int const _line_nbr, unsigned int const _column_nbr)
    : line_nbr(_line_nbr), column_nbr(_column_nbr),
      element(new float[_line_nbr * _column_nbr]) {}

Matrix::Matrix(Matrix const &m)
    : line_nbr(m.line_nbr), column_nbr(m.column_nbr),
...
Matrix operator*(float const a, Matrix const &m) { return m * a; }

Matrix &Matrix::operator+=(Matrix const &m) {}

Matrix &Matrix::operator*=(Matrix const &m) {}

bool Matrix::operator==(Matrix const &m) const { return false; }

Vector Matrix::extract_ligne(unsigned int const l, unsigned int const c1,
                             unsigned int const c2) const {
  return *(Vector *)NULL;
}

Vector Matrix::extract_col(unsigned int const l1, unsigned int const l2,
...
Matrix Matrix::extract_triangular_upper_diag() const { return *(Matrix *)NULL; }

Matrix Matrix::extract_diagonal() const { return *(Matrix *)NULL; }
