...
public:
  ...
  unsigned int get_line_nbr() const { return line_nbr; };

  /*! To access the number of columns. */
  unsigned int get_column_nbr() const { return column_nbr; };
  ...
  /*!
   * Initialize the matrix as the identity matrix (1 in the diagonal, 0
   * everywhere else).
   ...
   * \return A new Matrix whose coordinates are the sum of the coordinate of
   * this and m.
   */
   ...
   * \pre The two matrices should have the same size.
   * \return A new Matrix whose coordinates are the coordinate of this minus the
   * coordinates of m.
   */
  Matrix operator-(Matrix const &m) const;
   ...
   * \pre The number of columns of this should be equal to the number of lines
   * of m.
   * \return A new Matrix whose coordinates are the scalar product of lines of
   * this and column of m.
   */
  Matrix operator*(Matrix const &m) const;
   ...
   * \return A new Vector whose coordinates are the scalar product of lines of
   *this and v.
   */
   ...
   * \return A new matrix whose coordinates are the product of the coordinate of
   * this and a.
   */
   ...
   */
  Vector extract_ligne(unsigned int const l, unsigned int const c1,
   ...
   * \return An extracted copy.
   */
  Vector extract_col(unsigned int const l1, unsigned int const l2,
                     unsigned int const c) const;

  /*!
   * Extract a matrix that is equal under the diagonal, one on the diagonal and
  ...
  /*!
   * Extract a matrix that is equal above the diagonal and the diagonal and zero
   * everywhere else.
   * \pre must be a square matrix.
   */
  ...
  friend std::ostream &operator<<(std::ostream &ost, Matrix const &m);
};
 ...
 * \return A new matric whose coordinates are the product of the coordinate of m
 * and a.
 */
